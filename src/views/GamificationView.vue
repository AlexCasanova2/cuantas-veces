    ...
    <div v-if="isDebugMode" class="mb-6 p-4 bg-gray-100 rounded-lg">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="bg-white p-3 rounded">
          ...
          <p>XP: {{ gamificationStore.userLevel?.current_xp }}/{{ gamificationStore.userLevel?.xp_to_next }}</p>
        </div>
        <div class="bg-white p-3 rounded">
          <h3 class="font-medium mb-2">Estadísticas</h3>
          <p>Misiones Activas: {{ gamificationStore.activeMissions.length }}</p>
          <p>Logros Desbloqueados: {{ gamificationStore.userAchievements.length }}</p>
          <p>Tareas Completadas: {{taskStore.tasks.filter((t: TaskWithProgress) => t.progress === 100).length}}</p>
        </div>
      </div>
    </div>
  ...
  if (userStore.user?.id) {
    try {
      ...
      console.log('Datos cargados correctamente:', {
        ...
        userAchievements: gamificationStore.userAchievements,
